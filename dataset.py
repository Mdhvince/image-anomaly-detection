...
class MVTecDataset(torch.utils.data.Dataset):
    ...
    category,split,label,image_path,mask_path (schema in README.md).
    """

    def __init__(self, index_rows: list, category_root: Path, img_size: int, crop_size: int, preprocess: Callable) -> None:
        self.preprocess = preprocess
        self.img_size, self.crop_size = img_size, crop_size
        self.samples = [
            (
                category_root / row["image_path"],
                category_root / row["mask_path"] if row["mask_path"] else None,
                float(row["label"]),
            )
...
def build_dataloaders(index_csv: Path, category_name: str, img_size: int, crop_size: int, batch_size: int, valid_ratio: float, num_workers: int, preprocess: Callable) -> tuple:
    """Index CSV -> loaders for one category (train one model per category).

    :param index_csv: flat index produced by index_mvtec.
    :param category_name: MVTec-AD category to select.
    ...
    category_rows = index_frame[index_frame.category == category_name]
    category_root = index_csv.parent / category_name
    ...
    train_set = MVTecDataset(train_rows, category_root, img_size, crop_size, preprocess)
    test_set = MVTecDataset(test_rows, category_root, img_size, crop_size, preprocess)
